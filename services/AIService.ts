import { MovementService, TagService, AlertService } from './index';
...
class AIService {
  ...
  async generateInsights(): Promise<AIInsight[]> {
    try {
      ...
      const tags = await TagService.getAllAssignments();
      ...
      return insights.map(insight => ({
        ...insight,
        id: crypto.randomUUID ? crypto.randomUUID() : `insight-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        timestamp: Date.now()
      }));
    } catch (error) {
  ...
  async getInventoryRecommendations(): Promise<string> {
    try {
      ...
      for (const movement of movements) {
        const item = await TagService.getItemByTagId(movement.tagId, MOCK_ITEMS);
        if (item && item.id) {
